...
def process_slides(markdown_fn, output_fn):
...
def main():
    parser = argparse.ArgumentParser(description="""Create the HTML5 slide
        deck from a markdown source file""")
    parser.add_argument('-i', '--markdown', help='''The markdown source file.
        Default="slides.md"''', default='slides.md')
    parser.add_argument('-o', '--output', help='''The outut HTML file.
        Default="index.html"''', default='index.html')
    args = parser.parse_args()

    if not os.path.exists(args.markdown):
        raise OSError('The markdown file "%s" could not be found.' % args.markdown)

    process_slides(args.markdown, args.output)
